...
[horizontal]
*2.5.1.recommendation.1*:: The **`valid_range`** attribute must not be present if the **`valid_min`** and/or
**`valid_max`** attributes are present.
*2.5.1.recommendation.2*:: The **`_FillValue`** attribute must be the same type as its associated
variable.
*2.5.1.recommendation.3*:: The **`missing_value`** attribute must be the same type as its associated
variable.
*2.5.1.recommendation.4*:: The **`actual_range`** attribute must be of the same type as its associated
...
*2.5.1.recommendation.5*:: The **`actual_range`** attribute must have two elements, of which the first
...
*2.5.1.recommendation.6*:: There must not be an **`actual_range`** attribute if all the data values of
the associated variable equal the missing value.
*2.5.1.recommendation.7*:: If both the **`actual_range`** and **`valid_range/valid_min/valid_max`** are
